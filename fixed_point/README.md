...
* `src` folder with the source files for the code to be synthesized and the testbench
  * `div.h`, `div.cc`: header and source file for the code to be synthesized
...
* [ ] Explore the `src/div.h` and `src/div.cc` files to understand the code that will be synthesized into hardware.
...
* [ ] Check the generated reports in the `proj/solution/syn/report` folder to understand the performance and resource utilization of the synthesized design.
